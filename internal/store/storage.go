...
type Storage struct {
	...
	Pantry interface {
		GetAll() ([]PantryIngredient, error)
		Create(*PantryIngredient) (*PantryIngredient, error)
		Delete(*PantryIngredient) error
		DeleteByIngredientID(uint) error
		DeleteAll() error
		GetByIngredientID(uint) (*PantryIngredient, error)
	}
